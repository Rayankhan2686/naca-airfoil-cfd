"""
NACA 4412 CFD vs Reference Comparison
Compares OpenFOAM simpleFoam (k-omega SST, Re=2e5) results against the
XFOIL Re=200,000 (Ncrit=9) reference polar (airfoiltools.com
xf-naca4412-il-200000), the same exact-Re-matched benchmark methodology
used for NACA0012 / NACA2412 (see compare_naca0012.py, compare_naca2412.py).

The 4412 reference is real XFOIL: airfoiltools' precomputed xf-naca4412-il-200000
polar, cross-checked against a locally run XFOIL 6.99 (identical to <0.01 Cl).
"""
import csv
import os
import matplotlib
...
OUT_PATH = os.path.join(RESULTS_DIR, "NACA4412_vs_reference.png")
CSV_OUT = os.path.join(RESULTS_DIR, "NACA4412_vs_reference.csv")

cfd_csv = os.path.join(HOME, "OpenFOAM", "scripts", "results", "airfoil_results.csv")
xfoil_csv = os.path.join(HOME, "OpenFOAM", "scripts", "results", "xfoil_reference", "naca4412_re2e5_ncrit9.csv")

# ── Load OpenFOAM (CFD) data ──────────────────────────────────────────────
cfd = {}
with open(cfd_csv) as f:
    for row in csv.DictReader(f):
        if row["airfoil"] != "naca4412":
            continue
        if row.get("Cl", "") == "":
            continue
        a = float(row["alpha"])
...
_UNRELIABLE_NOTES = {"post-stall-unreliable", "no-steady-solution-unsteady-separation", "diverged-unreliable"}
reliable = [r for r in rows if r["note"] not in _UNRELIABLE_NOTES]
cl_errs = [abs(r["Cl_diff"]) for r in reliable]
cd_errs = [abs(r["Cd_diff"]) for r in reliable]
print(f"Matched {len(rows)} alphas ({len(reliable)} reliable, pre-stall).")
...
unrel = [r for r in rows if r["note"] in _UNRELIABLE_NOTES]
...
if unrel_a:
    ax1.plot(unrel_a, [cfd[a]["Cl"] for a in unrel_a], "x", color=RED, ms=8, mew=2,
             label="OpenFOAM breakdown α≥16° (no steady soln)")
...
colors3 = [RED if r["note"] in _UNRELIABLE_NOTES else AMBER for r in rows]
...
fig.suptitle(
    "NACA 4412 — OpenFOAM RANS vs XFOIL Reference  |  Re = 2×10⁵",
