"""
NACA 2412 CFD vs Reference Comparison
...
xf-naca2412-il-200000), the same exact-Re-matched benchmark methodology
used for NACA0012 (see compare_naca0012.py).
"""
import csv
import os
import matplotlib
...
OUT_PATH = os.path.join(RESULTS_DIR, "NACA2412_vs_reference.png")
CSV_OUT = os.path.join(RESULTS_DIR, "NACA2412_vs_reference.csv")

cfd_csv = os.path.join(HOME, "OpenFOAM", "scripts", "results", "airfoil_results.csv")
xfoil_csv = os.path.join(HOME, "OpenFOAM", "scripts", "results", "xfoil_reference", "naca2412_re2e5_ncrit9.csv")

# ── Load OpenFOAM (CFD) data ──────────────────────────────────────────────
cfd = {}
with open(cfd_csv) as f:
    for row in csv.DictReader(f):
        if row["airfoil"] != "naca2412":
            continue
        if row.get("Cl", "") == "":
            continue
        a = float(row["alpha"])
...
with open(xfoil_csv) as f:
    for line in f:
        ...
        try:
            ...
            continue  # header/metadata lines (e.g. "Alpha,Cl,Cd,...")
        xfoil[a] = {"Cl": cl, "Cd": cd}
...
if unrel_a:
    ax1.plot(unrel_a, [cfd[a]["Cl"] for a in unrel_a], "x", color=RED, ms=8, mew=2,
             label="OpenFOAM unreliable/no-steady-solution")
...
fig.suptitle(
    "NACA 2412 — OpenFOAM RANS vs XFOIL Reference  |  Re = 2×10⁵",
